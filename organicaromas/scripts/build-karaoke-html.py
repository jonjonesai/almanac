#!/usr/bin/env python3
"""Build HyperFrames index.html with per-word karaoke captions from WhisperX timings.

Config-driven (no hardcoded paths). Usage:
    build-karaoke-html.py <config.json>

config.json shape:
    {
      "brand": "organicaromas",
      "project_dir": "<repo-root>/organicaromas",
      "words_path": "assets/words.json",
      "voice_path": "assets/voice.mp3",
      "music_path": "assets/music.wav",
      "duration": 28.0,
      "brand_identity": { ... loaded from brand.json ... },
      "beats": [
        {"scene": "scene-1", "lo": 0, "hi": 13, "accent": [7,8], "bg": "freepik_3426796.jpg"},
        ...
      ],
      "scene_timings": [
        {"start": 0.0, "duration": 4.5},
        ...
      ]
    }

The beats[*].lo/hi index into words.json; accent is per-beat-relative word indices.
scene_timings has one entry per beat plus one for the outro.
"""
import json
import sys
...
"""
    ...
    fonts_url = (
        ...
        f"family={df_query}:ital,wght@0,500;0,600;1,500;1,600"
    ...
    <style>
      * {{ margin: 0; padding: 0; box-sizing: border-box; }}
      ...
      .scene-overlay {{
        ...
        background: linear-gradient(180deg, rgba(26,32,44,0) 0%, rgba(26,32,44,0) 50%, rgba(26,32,44,0.92) 95%);
        pointer-events: none;
      }}
      .caption {{
        position: absolute;
      ...
      #scene-title .title-text {{
        ...
        font-size: 92px; line-height: 1.05;
        color: #fafaf7;
        text-align: center;
        letter-spacing: -0.005em;
      }}
      ...
      #scene-outro .outro-title .accent-teal {{
        color: {primary};
        display: block;
        margin-top: 12px;
      }}
...
{scenes_html}
      <div id="scene-outro" class="scene">
        <div class="outro-title">
          {outro_top}
          <span class="accent-teal">{outro_bottom}</span>
        </div>
      ...
      <audio id="music" data-start="0" data-duration="{duration}" data-track-index="11" src="{cfg['music_path']}" data-volume="0.16"></audio>
    </div>
...
{words_js}
      ...
      const eases = ["power2.out", "expo.out", "power3.out", "sine.out"];


      scenes.forEach((sc, i) => {{
        if (sc.bgType === "title") {{
          gsap.set(sc.id, {{ opacity: 1 }});
          tl.fromTo(sc.id + " .title-text", {{ y: 22, opacity: 0, scale: 0.97 }}, {{ y: 0, opacity: 1, scale: 1.0, duration: 0.45, ease: "expo.out" }}, 0.05);
          tl.fromTo(sc.id + " .title-brandmark", {{ opacity: 0 }}, {{ opacity: 1, duration: 0.35, ease: "power2.out" }}, 0.20);
          tl.to(sc.id, {{ opacity: 0, duration: 0.25, ease: "sine.inOut" }}, Math.max(0, sc.duration - 0.18));
          return;
        }}
        tl.to(sc.id, {{ opacity: 1, duration: 0.7, ease: "sine.inOut" }}, sc.start);
        // Ken Burns only for static image bgs — video clips have their own motion.
        if (sc.bgType === "image") {{
          tl.fromTo(sc.id + " .scene-bg", {{ scale: 1.0 }}, {{ scale: 1.08, duration: sc.duration, ease: "sine.inOut" }}, sc.start);
        }}
        if (sc.id === "#scene-1") {{
          tl.from(sc.id + " .brand-tag", {{ y: -20, opacity: 0, duration: 0.6, ease: "power2.out" }}, sc.start + 0.1);
        }}
        if (sc.id === "#scene-outro") {{
          tl.from(sc.id + " .outro-title", {{ y: 40, opacity: 0, duration: 1.1, ease: "expo.out" }}, sc.start + 0.3);
          tl.from(sc.id + " .brand-mark", {{ opacity: 0, duration: 0.8, ease: "power2.out" }}, sc.start + 1.0);
        }}
        // Fade-out anchored to current scene's end (not next.start) so the last
        // word of each beat finishes at full opacity before the crossfade starts.
        // Was: next - 0.4 → last word ended mid-fade.
        if (i < scenes.length - 1) {{
          tl.to(sc.id, {{ opacity: 0, duration: 0.7, ease: "sine.inOut" }}, sc.start + sc.duration - 0.5);
        }} else {{
      ...
      words.forEach(w => {{
        tl.to(w.sel, {{ opacity: 1, y: 0, duration: 0.18, ease: "power2.out" }}, w.t);
        tl.fromTo(w.sel, {{ y: 8 }}, {{ y: 0, duration: 0.18, ease: "power2.out" }}, w.t);
      }});
