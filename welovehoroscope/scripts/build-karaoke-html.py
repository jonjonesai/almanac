  ...
  <head>
    ...
    <style>
      * {{ margin: 0; padding: 0; box-sizing: border-box; }}
      ...
      /* Accent: keep same font-family as body so line metrics stay consistent —
         no more mid-phrase wraps from Cinzel being ~40% wider per glyph.
         Distinctiveness comes from color + italic + uppercase + letter-spacing.
         Cinzel still shines on title card + outro (different selectors). */
      .word.accent {{
        color: {primary};
        font-style: italic;
        font-weight: 700;
...
{words_js}
      ...
      scenes.forEach((sc, i) => {{
        ...
        // Fade-out anchored to THIS scene's own end (not the next scene's start)
        // so the last word has full opacity until ~0.2s after it ends, then a
        // 0.5s fade-out completes at scene end. Pre/post-roll handles overlap.
