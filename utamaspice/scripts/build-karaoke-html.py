#!/usr/bin/env python3
"""Build UtamaSpice HyperFrames index.html with per-word karaoke captions.

Forked from CC build-karaoke-html.py 2026-05-12 (which itself was forked from OA).
Differences from CC:
- Slower easings (sine.inOut / power2.out) — wellness/artisan ritual pace, not hip-hop punch
- Fraunces italic accent words (no uppercase), lowercase rendering for accent serif feel
- Smaller Ken Burns scale (1.0 → 1.06) — gentler than CC's 1.10

Config-driven (same shape as OA/CC). Usage:
...
"""
    ...
    fonts_url = (
        ...
        f"family={df_query}:ital,opsz,wght@0,9..144,500;0,9..144,600;1,9..144,500;1,9..144,600"
    ...
    <style>
      * {{ margin: 0; padding: 0; box-sizing: border-box; }}
      ...
      .scene-overlay {{
        ...
        background: linear-gradient(180deg, rgba({bg_r},{bg_g},{bg_b},0) 0%, rgba({bg_r},{bg_g},{bg_b},0) 50%, rgba({bg_r},{bg_g},{bg_b},0.93) 95%);
        pointer-events: none;
      }}
      .caption {{
        position: absolute;
      ...
      #scene-outro .outro-title {{
        ...
        font-family: "{display_font}", serif;
        font-style: italic; font-weight: 600;
        font-size: 116px; line-height: 1.02;
        color: #f5efe3;
        text-align: center;
        letter-spacing: -0.01em;
      }}
...
{words_js}
      ...
      scenes.forEach((sc, i) => {{
        if (sc.bgType === "title") {{
          ...
          tl.fromTo(sc.id + " .title-text", {{ y: 22, opacity: 0, scale: 0.97 }}, {{ y: 0, opacity: 1, scale: 1.0, duration: 0.50, ease: "expo.out" }}, 0.05);
          tl.fromTo(sc.id + " .title-brandmark", {{ opacity: 0 }}, {{ opacity: 1, duration: 0.40, ease: "power2.out" }}, 0.22);
          tl.to(sc.id, {{ opacity: 0, duration: 0.28, ease: "sine.inOut" }}, Math.max(0, sc.duration - 0.20));
          return;
        }}
        ...
        if (sc.bgType === "image") {{
          tl.fromTo(sc.id + " .scene-bg", {{ scale: 1.0 }}, {{ scale: 1.06, duration: sc.duration, ease: "sine.inOut" }}, sc.start);
        }}
        if (sc.id === "#scene-1") {{
          tl.from(sc.id + " .brand-tag", {{ y: -20, opacity: 0, duration: 0.7, ease: "power2.out" }}, sc.start + 0.1);
        }}
        if (sc.id === "#scene-outro") {{
          tl.from(sc.id + " .outro-title", {{ y: 40, opacity: 0, duration: 1.2, ease: "expo.out" }}, sc.start + 0.3);
          tl.from(sc.id + " .brand-mark", {{ opacity: 0, duration: 0.9, ease: "power2.out" }}, sc.start + 1.1);
        }}
        // Fade-out anchored to current scene's end so last word of each beat
        // finishes at full opacity. Was: next - 0.4 → last word ate the fade.
